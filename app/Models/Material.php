<?php

namespace App\Models;
use App\Core\Database;
use PDO;


//Material = matéria-prima usada na fabricação dos produtos.
 
class Material
{
    private PDO $db;

    // Campos permitidos para ordenação. 
    private const ORDENAVEIS = ['nome', 'custo_unitario', 'estoque', 'criado_em'];

    public function __construct()
    {
        $this->db = Database::getConnection();
    }

    public function listar(array $opts = []): array
    {
        $sql = 'SELECT * FROM material WHERE 1=1';
        $bind = [];

        if (!empty($opts['busca'])) {
            $sql .= ' AND nome LIKE :busca';
            $bind['busca'] = '%' . $opts['busca'] . '%';
        }

        $ordenarPor = in_array($opts['ordenar'] ?? '', self::ORDENAVEIS, true)
            ? $opts['ordenar'] : 'nome';
        $direcao = strtoupper($opts['dir'] ?? 'ASC') === 'DESC' ? 'DESC' : 'ASC';
        $sql .= " ORDER BY {$ordenarPor} {$direcao}";

        $stmt = $this->db->prepare($sql);
        $stmt->execute($bind);
        return $stmt->fetchAll();
    }

    public function buscar(int $id): ?array
    {
        $stmt = $this->db->prepare('SELECT * FROM material WHERE id_material = :id');
        $stmt->execute(['id' => $id]);
        return $stmt->fetch() ?: null;
    }

    public function criar(array $d): int
    {
        $stmt = $this->db->prepare(
            'INSERT INTO material (nome, unidade_medida, custo_unitario, estoque)
             VALUES (:nome, :un, :custo, :estoque)'
        );
        $stmt->execute([
            'nome'    => $d['nome'],
            'un'      => $d['unidade_medida'] ?? 'un',
            'custo'   => $d['custo_unitario'] ?? 0,
            'estoque' => $d['estoque'] ?? 0,
        ]);
        return (int) $this->db->lastInsertId();
    }

    public function atualizar(int $id, array $d): bool
    {
        $stmt = $this->db->prepare(
            'UPDATE material SET nome = :nome, unidade_medida = :un,
             custo_unitario = :custo, estoque = :estoque WHERE id_material = :id'
        );
        return $stmt->execute([
            'id'      => $id,
            'nome'    => $d['nome'],
            'un'      => $d['unidade_medida'] ?? 'un',
            'custo'   => $d['custo_unitario'] ?? 0,
            'estoque' => $d['estoque'] ?? 0,
        ]);
    }

    public function excluir(int $id): bool
    {
        $stmt = $this->db->prepare('DELETE FROM material WHERE id_material = :id');
        return $stmt->execute(['id' => $id]);
    }

    public function contar(): int
    {
        return (int) $this->db->query('SELECT COUNT(*) FROM material')->fetchColumn();
    }
}

?>